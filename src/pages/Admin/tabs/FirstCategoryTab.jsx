...
import { getFirstCategoryCoefficients, updateFirstCategoryCoefficients } from '../../../api/coefficientsAPI';
...
const FirstCategoryTab = () => {
  ...
  const fetchCoefficients = async () => {
    ...
    setError(null);
    try {
      const data = await getFirstCategoryCoefficients();
  ...
  const handleSave = async () => {
    ...
    try {
      await updateFirstCategoryCoefficients(coefficients);
      setSuccess(true);
  ...
  return (
    <div>
      <h2 className="mb-4">Коэффициенты Первой Категории</h2>
